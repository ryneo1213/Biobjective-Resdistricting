...
with open(results_filename,'w',newline='') as csvfile:   
    ...
    my_fieldnames += ['max_pop', 'min_pop', 'pop_dev', 'max_dev']
...
for key in batch_configs.keys(): 
    ...
    max_dev = math.floor(deviation * (sum(population) / k))
    
    while(max_dev > 0 and statusCode == 2):
        print("L =",L,", U =",U,", k =",k)
        result['k'] = k
        result['L'] = L
        result['U'] = U
        ...
        if base == 'labeling':        
            ...
            newpopulation.add_pop_constraints(m, population, deviation, k, G, max_dev)
        ...
        if m.SolCount > 0:
            ...
            for j in range(k):
                ...
                print("District", j, "has a population of:", dist_pop[j])
            
            
            result['max_pop'] = max(dist_pop)
            result['min_pop'] = min(dist_pop)
            
            result['pop_dev'] = max(dist_pop) - min(dist_pop)
            max_dev = max(dist_pop) - min(dist_pop) - 1
